...
$arr_input_cache = $arr_whitelist_cache = $arr_output = [];
...
foreach ($formatterList as $name => $formatObj) {
    ...
    $arr_src_domains = [];
    $arr_tmp_whitelist = []; //單次的白名單列表
    if (
        is_array($formatObj["whitelist_attached"]) &&
        count($formatObj["whitelist_attached"]) > 0
    ) {
        foreach (
            $formatObj["whitelist_attached"]
            as $white_file => $white_attr
        ) {
            if (
                !array_key_exists(
                    "{$white_file}_{$white_attr["merge_mode"]}",
                    $arr_whitelist_cache
                )
            ) {
                $arr_attached = file(
                ...
                $arr_attached = array_fill_keys(
                    $arr_attached,
                    $white_attr["merge_mode"]
                );
                $arr_whitelist_cache[
                    "{$white_file}_{$white_attr["merge_mode"]}"
                ] = $arr_attached;
