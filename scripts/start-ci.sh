...
cat easylist*.txt | grep -E "^\|\|[^\*\^\/]+?\^" | grep -v -E "\^\*|\/" | sort -d -u | uniq >base-src-easylist.txt
cat easylist*.txt | grep -E "^\|\|?([^\^=\/:]+)?\*([^\^=\/:]+)?\^" | sort -d -u | uniq >wildcard-src-easylist.txt
cat easylist*.txt | grep -E "^@@?[^\^=\/:]+?\^([^\/=\*]+)?$" | sort -d -u | uniq >whiterule-src-easylist.txt
cat easylist0.txt | grep -E "^[^\|!]|\*|(^[^!]\S*[^\^]$)" | sort -d -u | uniq >e-easylist.txt
cat easylist0.txt | grep -E "^[^@!]\S*[^\^]$" | sort -d -u | uniq >>base-src-easylist.txt
#cat easylist0.txt | grep -E "\$(\S+,)*(client|dnstype|dnsrewrite|important|badfilter|ctag)" | sort -d -u | uniq >rule-modifiers.txt
sort -d -u base-src-easylist.txt | uniq >a.txt
...
(head -n 4 ../a.txt && tail -n +5 ../a.txt | sort -d -u) | uniq > ../anti-ad-easylist.txt
rm -f ../a.txt
